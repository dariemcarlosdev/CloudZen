        ...
        [HttpTrigger(AuthorizationLevel.Anonymous, "post", "options", Route = "chat")] HttpRequest req)
    {
        ...
        try
        {
            ...
            foreach (var msg in chatRequest.Messages)
            {
                ...
                // Only enforce content length and dangerous-content checks on user messages —
                // assistant messages are generated by the API itself and may exceed the user input limit.
                if (msg.Role == "user")
                {
                    var contentValidation = InputValidator.ValidateTextInput(
                        msg.Content, "Message", maxLength: MaxMessageContentLength);

                    if (!contentValidation.IsValid)
                    {
                        return new BadRequestObjectResult(new ChatResponse { Success = false, Error = contentValidation.ErrorMessage });
                    }
                }
            }
